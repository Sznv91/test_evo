...
public class ExpandableLayout extends FrameLayout {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        setVisibility(expansion == 0 && size == 0 && activeChildId == -1 ? GONE : VISIBLE);

        LinearLayout childContent = (LinearLayout) getChildAt(0);

        for (int i = 0; i < childContent.getChildCount(); i++) {
            View child = childContent.getChildAt(i);
            if (i != activeChildId) {
                if (child instanceof IExpanded)
                    ((IExpanded) child).expand(expansion);
            }
        }
    ...
    public void setActiveChildId(int activeChildId) {
    ...
    public void setExpansion(float expansion) {
        ...
        setVisibility(state == COLLAPSED && activeChildId == -1 ? GONE : VISIBLE);
